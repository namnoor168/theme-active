...
function ftc_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'colorscheme', array(
		'type'    => 'radio',
		'label'    => esc_html__( 'Color Scheme', 'corona' ),
		'choices'  => array(
			'light'  => esc_html__( 'Light', 'corona' ),
			'dark'   => esc_html__( 'Dark', 'corona' ),
			'custom' => esc_html__( 'Custom', 'corona' ),
		),
		'section'  => 'colors',
		'priority' => 5,
	) );
	...
	$wp_customize->add_section( 'theme_options', array(
		'title'    => esc_html__( 'Theme Options', 'corona' ),
	...
	$wp_customize->add_control( 'page_layout', array(
		...
		'type'        => 'radio',
		'description' => esc_html__( 'When the two column layout is assigned, the page title is in one column and content is in the other.', 'corona' ),
		'choices'     => array(
			'one-column' => esc_html__( 'One Column', 'corona' ),
			'two-column' => esc_html__( 'Two Column', 'corona' ),
		),
	...
	for ( $i = 1; $i < ( 1 + $num_sections ); $i++ ) {
		...
		$wp_customize->add_control( 'panel_' . $i, array(
			...
			'label'          => sprintf( esc_html__( 'Front Page Section %d Content', 'corona' ), $i ),
			'description'    => ( 1 !== $i ? '' : esc_html__( 'Select pages to feature in each area from the dropdowns. Add an image to a section by setting a featured image in the page editor. Empty sections will not be displayed.', 'corona' ) ),
			'section'        => 'theme_options',
			'type'           => 'dropdown-pages',
			'allow_addition' => true,
