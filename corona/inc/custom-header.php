...
function ftc_custom_header_setup() {
	...
	/**
	 * Filter corona custom-header support arguments.
	 *
	 * @since corona 1.0
	 *
	 * @param array $args {
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'corona' ),
		),
	) );
}
...
function ftc_video_controls( $settings ) {
	$settings['l10n']['play'] = '<span class="screen-reader-text">' . esc_html__( 'Play background video', 'corona' ) . '</span>' . ftc_get_svg( array( 'icon' => 'play' ) );
	$settings['l10n']['pause'] = '<span class="screen-reader-text">' . esc_html__( 'Pause background video', 'corona' ) . '</span>' . ftc_get_svg( array( 'icon' => 'pause' ) );
	return $settings;
}
